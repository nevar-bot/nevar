...
export default class MemeCommand extends BaseCommand
{
	...
	private async sendMeme(member: any): Promise<void>
	{
		...
		let memes: any = (await axios.get('https://www.reddit.com/r/ich_iel/top.json?sort=top&t=day&limit=1000', {
		...
		const collector: any = memeMessage.createMessageComponentCollector({ filter: ({ user }: any): boolean => user.id === member.user.id });
