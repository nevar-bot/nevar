...
export default class LetmegooglethatCommand extends BaseCommand {
		...
		super(client, {
			...
			slashCommand: {
				...
				data: new SlashCommandBuilder()
					.addStringOption((option: any) => option.setName("text").setDescription("Gib deine Suchanfrage ein").setRequired(true))
					.addUserOption((option: any) =>
						option.setName("nutzer").setDescription("Wähle für wen du die Suchanfrage durchführen möchtest").setRequired(false)
					)
			}
		});
	}
	...
	private async googleThat(text: string, user: any = null): Promise<void> {
		...
		const googleText: string = user
			? "Lass mich das für dich googlen, " + user.username + ": [{0}]({1})"
