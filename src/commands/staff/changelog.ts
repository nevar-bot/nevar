	...
	private async sendChangelog(): Promise<void> {
		...
		buttonCollector.on(
			...
			async (interaction: any): Promise<void> => {
				...
				const newActionRow: any = new ActionRowBuilder().addComponents(
					newInput
				);
				const fixedActionRow: any =
					new ActionRowBuilder().addComponents(fixedInput);
				const improvedActionRow: any =
					new ActionRowBuilder().addComponents(improvedInput);
				const removedActionRow: any =
				...
				interaction
					.awaitModalSubmit({
						filter: (int: any): boolean =>
							int.user.id === this.message.author.id,
						time: 10 * 60 * 1000
					})
