...
export default class ChangelogCommand extends BaseCommand
{
	...
	private async sendChangelog(): Promise<void>
	{
		...
		const embedMessage: any = await this.message.reply({ embeds: [createEmbed], components: [buttonRow] });

		const buttonCollector: any = embedMessage.createMessageComponentCollector({ filter: (i: any): boolean => i.user.id === this.message.author.id });
		buttonCollector.on("collect", async (interaction: any): Promise<void> =>
		{
			...
			interaction.awaitModalSubmit({ filter: (int: any): boolean => int.user.id === this.message.author.id, time: 10 * 60 * 1000 })
