...
export default class SearchserverCommand extends BaseCommand {
	...
	private async searchServer(id: string): Promise<void> {
		...
		const createdDate: string = moment(guild.createdTimestamp).format("DD.MM.YYYY HH:mm");
		const createdDiff: string = this.client.utils.getRelativeTime(guild.createdTimestamp);
		const invitedDate: string = moment(guild.joinedAt).format("DD.MM.YYYY HH:mm");
		const invitedDiff: string = this.client.utils.getRelativeTime(guild.joinedTimestamp);
