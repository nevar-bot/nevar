...
export default class TwitchnotifierCommand extends BaseCommand {
    ...
    async addNotifier(channel: string, data: any): Promise<void>{
        const authProvider: AppTokenAuthProvider = new AppTokenAuthProvider(this.client.config.apikeys["TWITCH_CLIENT_ID"], this.client.config.apikeys["TWITCH_CLIENT_SECRET"]);
        ...
        if(user){
            if(data.guild.settings.notifiers.twitch.channels.length >= 3){
                const errorEmbed: EmbedBuilder = this.client.createEmbed(
                    this.translate("administration/twitchnotifier:errors:limitExceeded"),
                    "error",
                    "error"
                );
            ...
            if (data.guild.settings.notifiers.twitch.channels.find((addedUser: any): boolean => addedUser.id === user.id)) {
                const errorEmbed: EmbedBuilder = this.client.createEmbed(
                    this.translate("administration/twitchnotifier:errors:alreadyAdded"),
                    "error",
                    "error"
                );
                return this.interaction.followUp({ embeds: [errorEmbed] });
            ...
            const successEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:added", { name: user.name, url: "https://twitch.tv/" + user.name }),
                "success",
                "success"
            );
            ...
            const errorEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:errors:cantFindChannel"),
                "error",
                "error",
                channel
            );
    ...
    async removeNotifier(channel: string, data: any): Promise<void> {
        ...
        if (!user || !channel) {
            const errorEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:errors:missingChannelName"),
                "error",
                "error"
            );
            return this.interaction.followUp({ embeds: [errorEmbed] });
        ...
        if (data.guild.settings.notifiers.twitch.channels.find((addedUser: any): boolean => addedUser.id === user.id)) {
            ...
            const successEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:removed", { name: user.name, url: "https://twitch.tv/" + user.name }),
                "success",
                "success"
            );
            successEmbed.setThumbnail(user.profilePictureUrl)
            ...
            const errorEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:errors:notAdded"),
                "error",
                "error"
            );
            return this.interaction.followUp({ embeds: [errorEmbed] });
    ...
    async listNotifiers(data: any): Promise<void> {
        ...
        this.client.utils.sendPaginatedEmbed(
            ...
            3,
            channels,
            this.translate("administration/twitchnotifier:list:title"),
            this.translate("administration/twitchnotifier:list:empty"),
            "link"
        );
    }
    ...
    async setStatus(status: string, data: any): Promise<void> {
        if (!["on", "off"].includes(status)) {
            const errorEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:errors:missingStatus"),
                "error",
                "error",
                status
            );
        ...
        if (data.guild.settings.notifiers.twitch.enabled === statusBool) {
            const errorEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:errors:statusAlready", { status: statusString }),
                "error",
                "error"
            );
            return this.interaction.followUp({ embeds: [errorEmbed] });
        ...
        const successEmbed: EmbedBuilder = this.client.createEmbed(
            this.translate("administration/twitchnotifier:statusSet", { status: statusString }),
            "success",
            "success"
        );
        return this.interaction.followUp({ embeds: [successEmbed] });
    ...
    async setChannel(channel: any, data: any): Promise<void> {
        if (!channel) {
            const errorEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:errors:missingChannel"),
                "error",
                "error"
            );
            return this.interaction.followUp({ embeds: [errorEmbed] });
        ...
        if (data.guild.settings.notifiers.twitch.announcementChannel === channel.id) {
            const errorEmbed: EmbedBuilder = this.client.createEmbed(
                this.translate("administration/twitchnotifier:errors:channelAlready", { channel: channel.toString() }),
                "error",
                "error"
            );
            return this.interaction.followUp({ embeds: [errorEmbed] });
        ...
        const successEmbed: EmbedBuilder = this.client.createEmbed(
            this.translate("administration/twitchnotifier:channelSet", { channel: channel.toString() }),
            "success",
            "success"
        );
        return this.interaction.followUp({ embeds: [successEmbed] });
