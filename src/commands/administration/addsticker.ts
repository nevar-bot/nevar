...
export default class AddstickerCommand extends BaseCommand
{
	...
	private async addSticker(url: string, name: string, emoji: string, description: string): Promise<void>
	{
		...
		if (!stringIsUrl(url) || !urlIsImage(url) || !stringIsEmoji(emoji) || !nodeEmoji.find(emoji)) {
			const invalidOptionsEmbed: EmbedBuilder = this.client.createEmbed("Du musst einen Link zu einem Bild und einen Standard-Emoji eingeben. Beachte zusätzlich, dass derzeit nicht alle Emojis unterstützt werden.", "error", "error");
		...
		try {
			...
			const successEmbed: EmbedBuilder = this.client.createEmbed("Der neue Sticker {0} wurde erstellt.", "success", "success", name);
			successEmbed.setThumbnail(url);
			return this.interaction.followUp({ embeds: [successEmbed] });
		} catch (e) {
			/* Error */
			const errorEmbed: EmbedBuilder = this.client.createEmbed("Beim Erstellen des Stickers ist ein unerwarteter Fehler aufgetreten.", "error", "error");
