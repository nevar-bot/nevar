...
export default class AimodCommand extends BaseCommand
{
		...
		super(client, {
			...
			slashCommand: {
				...
				data: new SlashCommandBuilder()
					...
					.addSubcommand((subcommand: any) => subcommand
						.setName("explain")
						.setDescription("Erklärt die AI-gestützte Chatmoderation")
					)
			}
		})
	}
	...
	public async dispatch(interaction: any, data: any): Promise<void>
	{
		...
		switch (subcommand) {
			case "status":
				...
				break;
			case "exclude":
				await this.exclude(interaction.options.getString("aktion"), interaction.options.getChannel("channel"), interaction.options.getRole("rolle"), data);
				break;
			case "threshold":
				await this.setThreshold(interaction.options.getNumber("wert"), data);
				break;
			case "channel":
	...
	private async setStatus(status: string, data: any): Promise<void>
	{
		...
		const embed: EmbedBuilder = this.client.createEmbed("Die AI-gestützte Chatmoderation wurde {0}", "success", "normal", status === "on" ? "aktiviert" : "deaktiviert");
	...
	private async exclude(action: string, channel: any, role: any, data: any): Promise<void>
	{
		if (action === "add") {
			if (!channel && !role) {
				const embed: EmbedBuilder = this.client.createEmbed("Du musst einen Channel oder eine Rolle angeben.", "error", "error");
				return this.interaction.followUp({ embeds: [embed] })
			}
			if (channel) {
				if (data.guild.settings.aiModeration.excludedChannels.includes(channel.id)) {
					const embed: EmbedBuilder = this.client.createEmbed("In {0} ist die AI-gestützte Chatmoderation bereits deaktiviert.", "error", "error", channel.toString());
					return this.interaction.followUp({ embeds: [embed] })
				}
				...
				const successEmbed: EmbedBuilder = this.client.createEmbed("In {0} ist die AI-gestützte Chatmoderation ab sofort deaktiviert.", "success", "success", channel.toString());
				return this.interaction.followUp({ embeds: [successEmbed] });
			}
			if (role) {
				if (data.guild.settings.aiModeration.excludedRoles.includes(role.id)) {
					const embed: EmbedBuilder = this.client.createEmbed("Für {0} ist die AI-gestützte Chatmoderation bereits deaktiviert.", "error", "error", role.toString());
					return this.interaction.followUp({ embeds: [embed] })
				}
				...
				const successEmbed: EmbedBuilder = this.client.createEmbed("Für {0} ist die AI-gestützte Chatmoderation ab sofort deaktiviert.", "success", "success", role.toString());
				return this.interaction.followUp({ embeds: [successEmbed] });
			}
		}
		...
		if (action === "remove") {
			...
			if (channel) {
				if (!data.guild.settings.aiModeration.excludedChannels.includes(channel.id)) {
					const embed: EmbedBuilder = this.client.createEmbed("In {0} ist die AI-gestützte Chatmoderation nicht deaktiviert.", "error", "error", channel.toString());
					return this.interaction.followUp({ embeds: [embed] })
				}
				data.guild.settings.aiModeration.excludedChannels = data.guild.settings.aiModeration.excludedChannels.filter((id: string): boolean => id !== channel.id);
				data.guild.markModified("settings.aiModeration.excludedChannels");
				await data.guild.save();

				const successEmbed: EmbedBuilder = this.client.createEmbed("In {0} ist die AI-gestützte Chatmoderation ab sofort aktiviert.", "success", "success", channel.toString());
				return this.interaction.followUp({ embeds: [successEmbed] });
			}
			if (role) {
				if (!data.guild.settings.aiModeration.excludedRoles.includes(role.id)) {
				...
				data.guild.settings.aiModeration.excludedRoles = data.guild.settings.aiModeration.excludedRoles.filter((id: string): boolean => id !== role.id);
				data.guild.markModified("settings.aiModeration.excludedRoles");
				await data.guild.save();

				const successEmbed: EmbedBuilder = this.client.createEmbed("Für {0} ist die AI-gestützte Chatmoderation ab sofort aktiviert.", "success", "success", role.toString());
				return this.interaction.followUp({ embeds: [successEmbed] });
			}
		}
		...
		if (action === "list") {
			...
			await this.client.utils.sendPaginatedEmbed(this.interaction, 10, excludedChannelsAndRoles, "Deaktivierte Channel und Rollen", "Es sind bisher keine Channel oder Rollen von der AI-gestützten Chatmoderation ausgeschlossen worden.", "");
		}
	}
	...
	private async setThreshold(number: any, data: any): Promise<void>
	{
		...
		const embed: EmbedBuilder = this.client.createEmbed("Die AI-gestützte Chatmoderation warnt nun ab einer Bewertung von {0}.", "success", "success", number);
		return this.interaction.followUp({ embeds: [embed] });
	}
	...
	private async setChannel(channel: any, data: any): Promise<void>
	{
		...
		const embed: EmbedBuilder = this.client.createEmbed("Die AI-gestützte Chatmoderation sendet Warnungen ab sofort in {0}.", "success", "success", channel.toString());
		return this.interaction.followUp({ embeds: [embed] });
	}
