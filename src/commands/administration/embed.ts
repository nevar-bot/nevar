...
export default class EmbedCommand extends BaseCommand {
				...
				data: new SlashCommandBuilder()
					...
					.addStringOption((option: any) =>
						option
							...
							.setDescription(
								'Gib die Beschreibung des Embeds ein'
							)
							.setRequired(false)
					)
					.addAttachmentOption((option: any) =>
					...
					.addStringOption((option: any) =>
						option
							.setName('farbe')
							.setDescription(
								'Gib die Farbe des Embeds im HEX-Format ein'
							)
							.setRequired(false)
					)
			}
	...
	private async createEmbed() {
		...
		const thumbnail: any =
			this.interaction.options.getAttachment('thumbnail');
		const image: any = this.interaction.options.getAttachment('bild');
		const footerText: string =
			this.interaction.options.getString('footertext');
		const footerIcon: any =
			this.interaction.options.getAttachment('footericon');
		const color: any =
			this.interaction.options.getString('farbe') ||
			this.client.config.embeds['DEFAULT_COLOR'];

		if (color && !this.client.utils.stringIsHexColor(color)) {
			const errorEmbed: EmbedBuilder = this.client.createEmbed(
				'Du musst eine Farbe im HEX-Format eingeben.',
				'error',
				'error'
			);
		...
		if (authorIcon && !authorIcon.contentType.startsWith('image/')) {
			const errorEmbed: EmbedBuilder = this.client.createEmbed(
				'Das Autor-Icon muss ein Bild sein.',
				'error',
				'error'
			);
			return this.interaction.followUp({ embeds: [errorEmbed] });
		...
		if (thumbnail && !thumbnail.contentType.startsWith('image/')) {
			const errorEmbed: EmbedBuilder = this.client.createEmbed(
				'Das Thumbnail muss ein Bild sein.',
				'error',
				'error'
			);
			return this.interaction.followUp({ embeds: [errorEmbed] });
		...
		if (image && !image.contentType.startsWith('image/')) {
			const errorEmbed: EmbedBuilder = this.client.createEmbed(
				'Das Embed-Bild muss ein Bild sein.',
				'error',
				'error'
			);
			return this.interaction.followUp({ embeds: [errorEmbed] });
		...
		if (footerIcon && !footerIcon.contentType.startsWith('image/')) {
			const errorEmbed: EmbedBuilder = this.client.createEmbed(
				'Das Footer-Icon muss ein Bild sein.',
				'error',
				'error'
			);
			return this.interaction.followUp({ embeds: [errorEmbed] });
		...
		if (webhook) {
			...
			const successEmbed: EmbedBuilder = this.client.createEmbed(
				'Das Embed wurde erstellt und gesendet.',
				'success',
				'success'
			);
			...
			const errorEmbed: EmbedBuilder = this.client.createEmbed(
				'Beim Erstellen des Webhooks ist ein Fehler aufgetreten.',
				'error',
				'error'
			);
			return this.interaction.followUp({ embeds: [errorEmbed] });
