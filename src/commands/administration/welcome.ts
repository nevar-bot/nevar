...
export default class WelcomeCommand extends BaseCommand
{
	...
	private async sendPreview(data: any): Promise<void>
	{
		...
		if (!data.guild.settings.welcome.channel || !this.client.channels.cache.get(data.guild.settings.welcome.channel)) {
			const noChannelEmbed: EmbedBuilder = this.client.createEmbed("Es wurde kein Channel für die Willkommensnachricht festgelegt.", "error", "error");
			return this.interaction.followUp({ embeds: [noChannelEmbed] });
		}
		if (!data.guild.settings.welcome.message) {
			const noMessageEmbed: EmbedBuilder = this.client.createEmbed("Es wurde keine Nachricht für die Willkommensnachricht festgelegt.", "error", "error");
	...
	private async setChannel(channel: any, data: any): Promise<void>
	{
		...
		const successEmbed: EmbedBuilder = this.client.createEmbed("Die Willkommensnachricht wird ab jetzt in {0} gesendet.", "success", "success", channel.toString());
		return this.interaction.followUp({ embeds: [successEmbed] });
	}
	...
	private async setType(type: any, data: any): Promise<void>
	{
		...
		if (data.guild.settings.welcome.type === type) {
			...
			const isAlreadyEmbed: EmbedBuilder = this.client.createEmbed("Die Willkommensnachricht wird bereits als {0} gesendet.", "error", "error", statusString);
			return this.interaction.followUp({ embeds: [isAlreadyEmbed] });
		}
		...
		const statusString: string = type === "embed" ? "Embed" : "Textnachricht";
		const successEmbed: EmbedBuilder = this.client.createEmbed("Die Willkommensnachricht wird ab jetzt als {0} gesendet.", "success", "success", statusString);
		return this.interaction.followUp({ embeds: [successEmbed] });
	}
	...
	private async setColor(color: any, data: any): Promise<void>
	{
		...
		if (data.guild.settings.welcome.type === "text") {
			const embedNotEnabled: EmbedBuilder = this.client.createEmbed("Der Typ der Willkommensnachricht muss auf Embed gesetzt sein.", "error", "error");
			return this.interaction.followUp({ embeds: [embedNotEnabled] });
