...
export default class ReactionroleCommand extends BaseCommand {
            ...
            slashCommand: {
                ...
                data: new SlashCommandBuilder()
                    ...
                    .addRoleOption((option: any) => option
                        .setName("rolle")
                        .setDescription("Wähle die Rolle, die vergeben werden soll")
                        .setRequired(true)
                    )
                    .addStringOption((option: any) => option
                        .setName("emoji")
    ...
    private async addReactionRole(channel: any, id: string, role: any, emote: string, data: any): Promise<void> {
        ...
        if(role.id === this.interaction.guild.roles.everyone.id){
            const everyoneEmbed: EmbedBuilder = this.client.createEmbed("Die @everyone Rolle kann nicht als eine Reactionrole hinzugefügt werden.", "error", "error");
            return this.interaction.followUp({ embeds: [everyoneEmbed] });
        }

        /* Role is managed */
        if(role.managed){
            const roleIsManagedEmbed: EmbedBuilder = this.client.createEmbed("Rollen welche durch eine Integration verwaltet werden, können nicht als Reactionrole hinzugefügt werden.", "error", "error");
        ...
        if(this.interaction.guild.members.me.roles.highest.position <= role.position){
            const roleIsTooHighEmbed: EmbedBuilder = this.client.createEmbed("Da {0} eine höhere oder gleiche Position wie meine höchste Rolle ({1}) hat, kann sie nicht als Reactionrole hinzugefügt werden.", "error", "error", role, this.interaction.guild.members.me.roles.highest);
        ...
        if(stringIsCustomEmoji(originEmote) && !this.client.emojis.cache.find((e: any): boolean => e.id === emote)){
            const unusableEmojiEmbed: EmbedBuilder = this.client.createEmbed("Der Emoji muss auf einem Server wo ich bin verfügbar sein.", "error", "error");
