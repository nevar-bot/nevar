...
export default class Youtubenotifier extends BaseCommand
{
			...
			slashCommand: {
				...
				data: new SlashCommandBuilder()
					...
					.addStringOption((option: any) => option
						...
						.setDescription("Gib hier die ID des YouTube-Kanals an")
						.setRequired(false)
					)
					.addStringOption((option: any) => option
						.setName("status")
	...
	private async addNotifier(channelId: string, data: any): Promise<void>
	{
		...
		if (channel) {
			if (data.guild.settings.notifiers.youtube.channels.length > 3) {
				const errorEmbed: EmbedBuilder = this.client.createEmbed("Es können maximal 3 Kanäle gleichzeitig überwacht werden.", "error", "error");
				return this.interaction.followUp({ embeds: [errorEmbed] });
			}
			...
			const successEmbed: EmbedBuilder = this.client.createEmbed("[{0}]({1}) wird ab jetzt überwacht.", "success", "success", channel.username, "https://youtube.com/channel/" + channelId);
	...
	private async removeNotifier(channelId: string, data: any): Promise<void>
	{
		...
		if (data.guild.settings.notifiers.youtube.channels.find((channel: any): boolean => channel.id === channelId)) {
			data.guild.settings.notifiers.youtube.channels = data.guild.settings.notifiers.youtube.channels.filter((channel: any): boolean => channel.id !== channelId);
			data.guild.markModified("settings.notifiers.youtube.channels");
			await data.guild.save();

			const channel: any = await this.getChannelNameFromId(channelId);
			const successEmbed: EmbedBuilder = this.client.createEmbed("[{0}]({1}) wird ab jetzt nicht mehr überwacht.", "success", "success", channel.username, "https://youtube.com/channel/" + channelId);
			return this.interaction.followUp({ embeds: [successEmbed] });
		} else {
	...
	private async listNotifiers(data: any): Promise<void>
	{
		...
		this.client.utils.sendPaginatedEmbed(this.interaction, 3, channels, "Überwachte Youtube-Kanäle", "Es werden derzeit keine Youtube-Kanäle überwacht", "link");
	}
	...
	private async setChannel(channel: any, data: any): Promise<void>
	{
		...
		if (data.guild.settings.notifiers.youtube.announcementChannel === channel.id) {
			const errorEmbed: EmbedBuilder = this.client.createEmbed("YouTube-Videos werden bereits in {0} gesendet.", "error", "error", channel.toString());
			return this.interaction.followUp({ embeds: [errorEmbed] });
		}
		...
		const successEmbed: EmbedBuilder = this.client.createEmbed("YouTube-Videos werden ab jetzt in {0} gesendet.", "success", "success", channel.toString());
		return this.interaction.followUp({ embeds: [successEmbed] });
	}
