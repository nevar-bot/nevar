...
export default class GoodbyeCommand extends BaseCommand
{
	...
	private async setStatus(status: any, data: any): Promise<void>
	{
		if (data.guild.settings.farewell.enabled === JSON.parse(status)) {
			...
			const isAlreadyEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht ist bereits {0}.", "error", "error", statusString);
		...
		const statusString: string = JSON.parse(status) ? "aktiviert" : "deaktiviert";
		const successEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht wurde {0}.", "success", "success", statusString);
	...
	private async sendPreview(data: any): Promise<void>
	{
		if (!data.guild.settings.farewell.enabled) {
			const notEnabledEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht ist nicht aktiviert.", "error", "error");
		...
		if (!data.guild.settings.farewell.channel || !this.client.channels.cache.get(data.guild.settings.farewell.channel)) {
			const noChannelEmbed: EmbedBuilder = this.client.createEmbed("Es wurde kein Channel für die Verabschiedungsnachricht festgelegt.", "error", "error");
			return this.interaction.followUp({ embeds: [noChannelEmbed] });
		}
		if (!data.guild.settings.farewell.message) {
			const noMessageEmbed: EmbedBuilder = this.client.createEmbed("Es wurde keine Nachricht für die Verabschiedungsnachricht festgelegt.", "error", "error");
			return this.interaction.followUp({ embeds: [noMessageEmbed] });
		}
		if (!data.guild.settings.farewell.type) {
			const noTypeEmbed: EmbedBuilder = this.client.createEmbed("Es wurde kein Typ für die Verabschiedungsnachricht festgelegt.", "error", "error");
		...
		const testExecutedEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht wurde gesendet.", "success", "success");
	...
	private async setChannel(channel: any, data: any): Promise<void>
	{
		...
		const successEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht wird ab jetzt in {0} gesendet.", "success", "success", channel.toString());
		return this.interaction.followUp({ embeds: [successEmbed] });
	}
	...
	private async setType(type: any, data: any): Promise<any>
	{
		...
		if (data.guild.settings.farewell.type === type) {
			...
			const isAlreadyEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht wird bereits als {0} gesendet.", "error", "error", statusString);
			return this.interaction.followUp({ embeds: [isAlreadyEmbed] });
		}
		...
		const statusString: string = type === "embed" ? "Embed" : "Textnachricht";
		const successEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht wird ab jetzt als {0} gesendet.", "success", "success", statusString);
		return this.interaction.followUp({ embeds: [successEmbed] });
	}
	...
	private async setMessage(message: string, data: any): Promise<void>
	{
		...
		const successEmbed: EmbedBuilder = this.client.createEmbed("Die Verabschiedungsnachricht wurde geändert.", "success", "success");
		return this.interaction.followUp({ embeds: [successEmbed] });
	}
	...
	private async setColor(color: any, data: any): Promise<void>
	{
		...
		if (data.guild.settings.farewell.type === "text") {
			const embedNotEnabled: EmbedBuilder = this.client.createEmbed("Der Typ der Verabschiedungsnachricht muss auf Embed gesetzt sein.", "error", "error");
			...
			if (!this.client.utils.stringIsHexColor(color)) {
				const invalidColorEmbed: EmbedBuilder = this.client.createEmbed("Du musst eine gültige Farbe im HEX-Format angeben.", "error", "error");
