...
export default class AutoreactCommand extends BaseCommand {
    ...
    private async addAutoReact(data: any, channel: any, emote: string): Promise<void> {
        ...
        if(data.guild.settings.autoreact.find((r: any): boolean => r.channel === channel.id && r.emoji === emote)){
            const alreadyAddedEmbed: EmbedBuilder = this.client.createEmbed("Dieser Emoji ist in {0} bereits zum Autoreact hinzugefügt.", "error", "error", channel);
        ...
        const successEmbed: EmbedBuilder = this.client.createEmbed("{0} wurde in {1} zum Autoreact hinzugefügt.", "success", "success", originEmote, channel);
    ...
    private async removeAutoReact(data: any, channel: any, emote: string): Promise<void> {
        ...
        if(!data.guild.settings.autoreact.find((r: any): boolean => r.channel === channel.id && r.emoji === emote)){
            const alreadyAddedEmbed: EmbedBuilder = this.client.createEmbed("Dieser Emoji ist in {0} nicht zum Autoreact hinzugefügt.", "error", "error", channel);
            return this.interaction.followUp({ embeds: [alreadyAddedEmbed] });
        }
        ...
        const successEmbed: EmbedBuilder = this.client.createEmbed("{0} wurde in {1} vom Autoreact entfernt.", "success", "success", originEmote, channel);
        return this.interaction.followUp({ embeds: [successEmbed] });
    }
