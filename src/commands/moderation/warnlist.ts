...
export default class WarnlistCommand extends BaseCommand
{
	...
	private async listWarnings(user: any): Promise<void>
	{
		...
		for (let warn of warnings) {
			indicator++;
			const text: string =
				"### " + this.client.emotes.ban + " Warn " + indicator + "\n" +
				this.client.emotes.arrow + " Moderator: " + warn.moderator + "\n" +
				this.client.emotes.arrow + " Begründung: " + warn.reason + "\n" +
				this.client.emotes.arrow + " Verwarnt am: " + moment(warn.date).format("DD.MM.YYYY, HH:mm") + "\n";
			warnList.push(text);
		}

		await this.client.utils.sendPaginatedEmbed(this.interaction, 5, warnList, "Warns von " + member.user.username + " (" + warnCount + ")", member.user.username + " hat keine Verwarnungen", null);
	}
}