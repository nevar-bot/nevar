...
export default class WarnCommand extends BaseCommand
{
	...
	private async warnMember(user: any, reason: string): Promise<void>
	{
		...
		if (member.roles.highest.position >= this.interaction.member.roles.highest.position) {
			const higherRoleEmbed: EmbedBuilder = this.client.createEmbed("Du kannst keine Mitglieder verwarnen, die eine höhere Rolle haben als du.", "error", "error");
		...
		const privateText: string =
			"### " + this.client.emotes.ban + " Du wurdest auf " + this.interaction.guild.name + " verwarnt.\n\n" +
		...
		const logText: string =
			"### " + this.client.emotes.ban + " " + member.user.username + " wurde verwarnt\n\n" +
