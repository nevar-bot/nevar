...
import { SlashCommandBuilder, EmbedBuilder, Embed, ButtonBuilder } from "discord.js";
...
export default class MuteCommand extends BaseCommand
{
	...
	private async mute(member: any, reason: string, duration: string, data: any): Promise<void>
	{
		...
		if (!data.guild.settings.muterole || !this.interaction.guild.roles.cache.get(data.guild.settings.muterole)) {
			const noMuteRoleEmbed: EmbedBuilder = this.client.createEmbed("Es ist keine Mute-Rolle eingestellt!", "error", "error");
		...
		if (member.user.id === this.interaction.user.id) {
			const selfEmbed: EmbedBuilder = this.client.createEmbed("Du kannst dich nicht selbst muten.", "error", "error");
			return this.interaction.followUp({ embeds: [selfEmbed] });
		}

		if (member.user.id === this.client.user!.id) {
			const meEmbed: EmbedBuilder = this.client.createEmbed("Ich kann mich nicht selbst muten.", "error", "error");
		...
		if (member.roles.highest.position >= this.interaction.member.roles.highest.position) {
			const higherRoleEmbed: EmbedBuilder = this.client.createEmbed("Du kannst keine Mitglieder muten, die eine höhere Rolle haben als du.", "error", "error");
			return this.interaction.followUp({ embeds: [higherRoleEmbed] });
		}

		const muteRole: any = this.interaction.guild.roles.cache.get(data.guild.settings.muterole);
		if ([...muteRole.members].find((mutedUser): boolean => mutedUser[0] === member.user.id)) {
			const alreadyMutedEmbed: EmbedBuilder = this.client.createEmbed("{0} ist bereits gemutet.", "error", "error", member.user.username);
		...
		let unmuteDate: string = moment(Date.now() + mute.duration).format("DD.MM.YYYY, HH:mm");
		if (mute.duration === 200 * 60 * 60 * 24 * 365 * 1000) {
			unmuteDate = "/";
		}

		const areYouSureEmbed: EmbedBuilder = this.client.createEmbed("Bist du dir sicher, dass du {0} muten möchtest?", "arrow", "warning", member.user.username);
		...
		const confirmationAskMessage: any = await this.interaction.followUp({ embeds: [areYouSureEmbed], components: [buttonRow] });

		const confirmationButtonCollector: any = confirmationAskMessage.createMessageComponentCollector({ filter: (i: any): boolean => i.user.id === this.interaction.user.id, time: 1000 * 60 * 5, max: 1 });
				...
				case "confirm":
					mute.victim.roles.add(data.guild.settings.muterole, "MUTE - Dauer: " + relativeTime + " | Grund: " + mute.reason + " | Moderator: " + this.interaction.user.username + " | Unmute am: " + unmuteDate)
						.then(async (): Promise<void> =>
						{
							...
							const privateMuteEmbed: EmbedBuilder = this.client.createEmbed(privateText, null, "error", this.interaction.guild.name);
							...
							const text: string =
								...
								this.client.emotes.text + " Begründung: " + mute.reason;
							const logEmbed: EmbedBuilder = this.client.createEmbed(text, null, "normal");
							logEmbed.setTitle(this.client.emotes.timeout + " " + mute.victim.user.username + " wurde gemutet");
							...
							const publicText: string =
								...
								this.client.emotes.arrow + " Moderator: " + this.interaction.user.username + "\n" +
								this.client.emotes.arrow + " Unmute am: " + unmuteDate;
							const publicMuteEmbed: EmbedBuilder = this.client.createEmbed(publicText, null, "error", mute.victim.user.username);
						...
						{
							const cantMuteEmbed: EmbedBuilder = this.client.createEmbed("Ich konnte {0} nicht muten.", "error", "error", mute.victim.user.username);
							await clicked.update({ embeds: [cantMuteEmbed], components: [] });
						});
					break;
				case "decline":
					const declineEmbed: EmbedBuilder = this.client.createEmbed("{0} wurde nicht gemutet.", "error", "error", mute.victim.user.username);
