...
export default class BanCommand extends BaseCommand {
	...
	private async ban(
		...
		let relativeTime = this.client.utils.getRelativeTime(
			Date.now() - ban.duration
		);
		if (ban.duration === 200 * 60 * 60 * 24 * 365 * 1000) {
			relativeTime = 'Permanent';
		}
		let unbanDate: string = moment(Date.now() + ban.duration).format(
			'DD.MM.YYYY, HH:mm'
		);
		if (ban.duration === 200 * 60 * 60 * 24 * 365 * 1000) {
			unbanDate = '/';
		...
		const confirmationButtonCollector =
				...
				switch (confirmation) {
					case 'confirm':
						const privateText: string =
							...
							' Unban am: ' +
							unbanDate;
						const privateBanEmbed: EmbedBuilder =
						...
						try {
							...
							const publicText: string =
								...
								' Unban am: ' +
								unbanDate;
							const publicBanEmbed: EmbedBuilder =
								this.client.createEmbed(
									publicText,
									null,
									'error',
							...
							const cantBanEmbed: EmbedBuilder =
								this.client.createEmbed(
