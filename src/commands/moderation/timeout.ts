...
export default class TimeoutCommand extends NevarCommand {
	...
	private async timeout(targetMember: any, givenReason: string, duration: string): Promise<any> {
		...
		targetMember.timeout(durationInMs, moderator.user.username + " - " + reason)
			.then(async (): Promise<void> => {
				...
				await this.interaction.followUp({ embeds: [publicMessageEmbed], components: [] });
			})
			.catch(async (): Promise<void> => {
				...
				await this.interaction.followUp({ embeds: [timeoutFailedEmbed], components: [] });
			});
	}
}