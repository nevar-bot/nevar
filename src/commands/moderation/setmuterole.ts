...
export default class SetmuteroleCommand extends BaseCommand
{
	...
	private async setMuteRole(role: any, data: any): Promise<void>
	{
		...
		if (role.id === this.interaction.guild.roles.everyone.id) {
			const everyoneEmbed: EmbedBuilder = this.client.createEmbed("Die @everyone Rolle kann nicht als Mute-Rolle gesetzt werden.", "error", "error");
			return this.interaction.followUp({ embeds: [everyoneEmbed] });
		}

		/* Role is managed by an integration */
		if (role.managed) {
			const roleIsManagedEmbed: EmbedBuilder = this.client.createEmbed("Rollen welche durch eine Integration verwaltet werden, können nicht als Mute-Rolle gesetzt werden.", "error", "error");
		...
		if (this.interaction.guild.members.me.roles.highest.position <= role.position) {
			const roleIsTooHighEmbed: EmbedBuilder = this.client.createEmbed("Da {0} eine höhere oder gleiche Position wie meine höchste Rolle ({1}) hat, kann sie nicht als Mute-Rolle gesetzt werden.", "error", "error", role.toString(), this.interaction.guild.members.me.roles.highest.toString());
		...
		if (data.guild.settings.muterole === role.id) {
			const roleIsAlreadyMuteRoleEmbed: EmbedBuilder = this.client.createEmbed("{0} ist bereits die Mute-Rolle.", "error", "error", role.toString());
