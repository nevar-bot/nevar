...
export default class StaffsCommand extends BaseCommand
{
	...
	private async addStaff(args: any[]): Promise<void>
	{
		...
		if (!['head-staff', 'staff'].includes(args[1].toLowerCase())) {
			const invalidOptionsEmbed: EmbedBuilder = this.client.createEmbed("Du musst entweder \"staff\" oder \"head-staff\" als Staff-Typ angeben.", "error", "error");
			return this.message.reply({ embeds: [invalidOptionsEmbed] });
		}
		...
		const successEmbed: EmbedBuilder = this.client.createEmbed("{0} wurde als {1} hinzugefügt.", "success", "success", member.user.username, string);
	...
	private async listStaffs(): Promise<void>
	{
		...
		const embed: EmbedBuilder = this.client.createEmbed("Folgend sind alle Bot-Staffs aufgelistet:\n\n{0} {1}", "arrow", "normal", this.client.emotes.shine2, staffs.join("\n" + this.client.emotes.shine2 + " "));
