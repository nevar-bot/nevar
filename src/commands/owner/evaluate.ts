...
export default class EvaluateCommand extends BaseCommand {
    ...
    private async evaluate(code: string): Promise<any> {
        ...
        if (blacklist.some((term: string) => code.includes(term))) {
            const errorEmbed: EmbedBuilder = this.client.createEmbed("Der Code enthält nicht erlaubte Begriffe.", "error", "error");
            ...
            try {
                ...
                const deleteButton: ButtonBuilder = this.client.createButton("delete", "Response löschen", "Secondary", this.client.emotes.delete, false);
                const buttonRow: any = this.client.createMessageComponentsRow(deleteButton);
                const responseMessage: any = await this.message.channel.send({ content: `\`\`\`js\n${JSON.stringify(response, null, 2)}\`\`\``, components: [buttonRow] });

                const buttonCollector = responseMessage.createMessageComponentCollector({ filter: (i: any): boolean => i.user.id === this.message.author.id, time: 60000 });
