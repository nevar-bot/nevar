...
export default class UpdateinteractionsCommand extends BaseCommand
{
	...
	private async updateInteractions()
	{
		...
		if (res.success) {
			const successEmbed: EmbedBuilder = this.client.createEmbed("{0} Slash-Commands und Kontext-Menüs wurden aktualisiert.", "slashcommand", "success", res.interactionsRegistered);
			return this.message.reply({ embeds: [successEmbed] });
		} else {
			const errorEmbed: EmbedBuilder = this.client.createEmbed("Beim Aktualisieren der Slash-Commands und Kontext-Menüs ist ein Fehler aufgetreten.", "error", "error");
