...
class FloodGame extends BaseGame {
    ...
    public constructor(options: any = {}) {
        ...
        options.emojis = ['🟥', '🟦', '🟧', '🟪', '🟩'];
        this.squares.splice(0, 5, ...options.emojis);
    ...
    public async startGame(): Promise<void> {
        ...
        const embed: EmbedBuilder = this.options.client.createEmbed("Züge: " + this.turns + "/" + this.maxTurns + "\n\n" + this.getBoardContent(), "arrow", "normal")
        ...
        const msg: any = await this.sendMessage({ embeds: [embed], components: [row] });
        const collector = msg.createMessageComponentCollector({ filter: (btn: any): boolean => btn.user.id === this.interaction.user.id });
    ...
    private endGame(msg: any, result: any): any {
        const GameOverMessage: string = result ? "Du hast das Spiel nach **{turns}** Zügen gewonnen." : "Du hast das Spiel nach **{turns}** Zügen verloren.";

        const embed: EmbedBuilder = this.options.client.createEmbed("Spiel beendet\n" + this.options.client.emotes.arrow + " " + GameOverMessage.replace("{turns}", String(this.turns)) + "\n\n" + this.getBoardContent(), "rocket", "normal");
