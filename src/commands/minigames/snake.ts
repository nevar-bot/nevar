...
class SnakeGame extends BaseGame
{
	...
	private updateFoodLoc(): void
	{
		...
		do {
			applePos = { x: parseInt(String(Math.random() * 15)), y: parseInt(String(Math.random() * 10)) };
	...
	public async startGame(): Promise<void>
	{
		...
		const snakeEmbed: EmbedBuilder = this.client.createEmbed(" Punkte: " + this.score + "\n\n" + this.getBoardContent(undefined), "arrow", "normal");
		...
		const msg: any = await this.sendMessage({ embeds: [snakeEmbed], components: [row1, row2] });
	...
	{
		const gameOverEmbed: EmbedBuilder = this.client.createEmbed("Das Spiel ist vorbei, du hast " + this.score + " Punkte erreicht.\n\n" + this.getBoardContent(true), "arrow", "normal");
	...
	{
		const snakeCollector: any = msg.createMessageComponentCollector({ filter: (btn: any): boolean => btn.user.id === this.interaction.user.id });
