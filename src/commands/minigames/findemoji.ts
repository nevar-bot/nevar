...
class FindemojiGame extends BaseGame
{
	...
	public async startGame(): Promise<void>
	{
		...
		const findEmojiEmbed: EmbedBuilder = this.client.createEmbed("Du hast 5 Sekunden, um dir die Emojis in richtiger Reihenfolge zu merken!", "arrow", "normal");
		findEmojiEmbed.setThumbnail(this.client.user!.displayAvatarURL());

		const msg: any = await this.sendMessage({ embeds: [findEmojiEmbed], components: this.getComponents(true) });
		...
		const timeoutCallback: any = async (): Promise<void> =>
		{
			...
			const emojiCollector = msg.createMessageComponentCollector({ filter: (btn: any): boolean => btn.user.id === this.interaction.user.id, idle: 30000 });
	...
	private getComponents(showEmoji: any): any
	{
		...
		for (let x: number = 0; x < 2; x++) {
			...
			for (let y: number = 0; y < 4; y++) {
				...
				const btn: ButtonBuilder = this.client.createButton("findEmoji_" + (x * 4 + y), "\u200b", (buttonEmoji === this.selected ? (this.selected === this.emoji ? 'Success' : 'Danger') : "Primary"), (showEmoji ? buttonEmoji : null));
