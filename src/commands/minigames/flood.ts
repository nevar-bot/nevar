...
class FloodGame extends BaseGame {
		...
		for (let y: number = 0; y < this.length; y++) {
			for (let x: number = 0; x < this.length; x++) {
				this.gameBoard[y * this.length + x] =
					this.squares[
						Math.floor(Math.random() * this.squares.length)
					];
			}
		}
	}
	...
	private async updateGame(selected: any, msg: any): Promise<any> {
		...
		while (queue.length > 0) {
			...
			if (
				!block ||
				visited.some((v) => v.x === block.x && v.y === block.y)
			)
				continue;
			...
			if (this.gameBoard[index] === firstBlock) {
				...
				if (
					!visited.some((v) => v.x === up.x && v.y === up.y) &&
					up.y >= 0
				)
					queue.push(up);
		...
		for (let y: number = 0; y < this.length; y++) {
			for (let x: number = 0; x < this.length; x++) {
				if (this.gameBoard[y * this.length + x] !== selected)
					gameOver = false;
			}
		}

		if (this.turns >= this.maxTurns && !gameOver)
			return void this.endGame(msg, false);
