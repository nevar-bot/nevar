...
export default class LeaderboardCommand extends BaseCommand
{
	...
	{
		const leaderboardData: any[] = [... await this.client.levels.computeLeaderboard(this.client, await this.client.levels.fetchLeaderboard(this.interaction.guild.id, 10), true)];
		...
		for (let user of leaderboardData) {
			...
			beautifiedLeaderboard.push(
				"### " + emote + " **" + user.username + "**\n" + this.client.emotes.shine2 + " Level " + user.level + "\n" + this.client.emotes.shine2 + " " + this.client.format(user.xp) + " / " + this.client.format(this.client.levels.xpFor(user.level + 1)) + " XP"
			)
		}
