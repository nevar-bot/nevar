	...
	private async sendStats(): Promise<void>
	{
		...
		for (let userdata of staffsdata) {
			...
			if (userdata.staff.role === "head-staff") {
				if (!head_staffs.includes(user.username)) head_staffs.push(user.username);
			} else if (userdata.staff.role === "staff") {
				if (!head_staffs.includes(user.username) && !staffs.includes(user.username)) staffs.push(user.username);
			}
		}


		// @ts-ignore - Object is possibly 'null'
		const uptime: any = this.client.utils.getRelativeTime(Date.now() - this.client.uptime);
