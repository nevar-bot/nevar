...
export default class HelpCommand extends BaseCommand
{
	...
	private async showHelp(data: any): Promise<void>
	{
		...
		const description: string =
			"### " + this.client.emotes.globe + " " + this.client.user!.username + " ist ein Discord-Bot, der auf aktuell mehr als " + (Math.floor(this.client.guilds.cache.size / 10) * 10) + " Servern genutzt wird.\n\n" +
			this.client.emotes.discover + " Hier findest du eine Liste aller Befehle, welche du nutzen kannst.\n\n" +
			this.client.emotes.slashcommand + " Jeder Befehl beginnt mit dem **Slash (/)**.\n" +
			this.client.emotes.question + " Um Hilfe zu einem spezifischen Befehl zu erhalten, hänge den gewünschten Befehl an den Hilfe-Command an.";
		...
		const categoryActionRow: any = new ActionRowBuilder()
	...
	private async showHelpForCommand(command: string): Promise<any>
	{
		...
		if (clientCommand) {
			...
			if (clientCommand.conf.memberPermissions.length > 0) {
				helpString += this.client.emotes.user + " **Benötigte Rechte (Nutzer):** \n" + clientCommand.conf.memberPermissions.map((p: any): string => this.client.emotes.arrow + " " + this.client.permissions[p]).join("\n") + "\n\n";
			}

			if (clientCommand.conf.botPermissions.length > 0) {
				helpString += this.client.emotes.bot + " **Benötigte Rechte (Bot):** \n" + clientCommand.conf.botPermissions.map((p: any): string => this.client.emotes.arrow + " " + this.client.permissions[p]).join("\n") + "\n\n";
			}
