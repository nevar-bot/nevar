...
export default class DeletedataCommand extends BaseCommand {
	...
	private async deleteData(
		...
		const filter: any = (button: any): boolean =>
			button.user.id === this.interaction.user.id;
		const collector: any =
		...
		collector.on("collect", async (button: any): Promise<any> => {
			...
			if (type === "member") {
				...
				await this.client.deleteMember(
					this.interaction.user.id,
			...
			if (type === "guild") {
				...
				if (blockedOld.state) {
					const newGuilddata: any =
						await this.client.findOrCreateGuild(
							this.interaction.guild.id
						);
