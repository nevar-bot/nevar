...
export default class TimestampCommand extends BaseCommand
{
	...
	private async createTimestamp(date: string, type: string): Promise<void>
	{
		...
		if (!unix) {
			const errorEmbed: EmbedBuilder = this.client.createEmbed("Du hast kein gültiges Datum angegeben! Dieses muss aus einem Datum und einer Uhrzeit, nur einem Datum oder nur einer Uhrzeit bestehen.", "error", "normal");
			return this.interaction.followUp({ embeds: [errorEmbed] });
		}
		const timestamp: string = "<t:" + unix + ":" + type + ">"
		const rawTimestamp: string = "`<t:" + unix + ":" + type + ">`"
		const timestampEmbed: EmbedBuilder = this.client.createEmbed("Hier ist dein generierter Zeitstempel:\n{0} {1}\n{2} {3}", "success", "normal", this.client.emotes.calendar, timestamp, this.client.emotes.text, rawTimestamp);
		...
		const collector: any = this.interaction.channel.createMessageComponentCollector({ filter });
