...
export default class WeatherCommand extends BaseCommand
{
	...
	private async showWeather(city: string): Promise<void>
	{
		if (!this.client.config.apikeys["WEATHER"] || this.client.config.apikeys["WEATHER"] === "") {
			const noApiKeyEmbed: EmbedBuilder = this.client.createEmbed("Da in der Bot-Config der nötige Openweathermap-API-Key nicht hinterlegt wurde, kann der Weather-Befehl nicht genutzt werden.", "error", "error");
		...
		if (weatherInformation?.cod === 200) {
			...
			const errorEmbed: EmbedBuilder = this.client.createEmbed("Es konnte kein Ort mit dem Namen \"" + city + "\" gefunden werden.", "error", "error");
