...
class Levels {
    ...
    static async createUser(userId: string, guildId: string): Promise<any> {
        const isUser: any = await LevelSchema.findOne({ userID: userId, guildID: guildId });
        ...
        await newUser.save().catch((e: any): boolean => { return false });
        return newUser;
    }

    static async deleteUser(userId: string, guildId: string): Promise<any> {
        const user: any = await LevelSchema.findOne({ userID: userId, guildID: guildId });
        if(!user) return false;

        await LevelSchema.findOneAndDelete({ userID: userId, guildID: guildId }).catch((e: any): boolean => { return false });
        return user;
    }
    ...
    static async appendXp(userId: string, guildId: string, xp: number): Promise<any> {
        ...
        await user.save().catch((e: any): boolean => { return false });
    ...
    static async appendLevel(userId: string, guildId: string, levels: number): Promise<any> {
        ...
        user.save().catch((e: any): boolean => { return false });
    ...
    static async fetchLeaderboard(guildId: string, limit: number = 10): Promise<any> {
        return await LevelSchema.find({ guildID: guildId }).sort([["xp", "descending"]]).exec();
    }
    ...
    static async computeLeaderboard(client: any, leaderboard: any[], fetchUsers = false): Promise<any> {
        ...
        if(fetchUsers){
            for(const key of leaderboard){
                const user = await client.users.fetch(key.userID) || { username: "Unbekannt", displayName: "Unbekannt" };
                computedArray.push({
                    guildID: key.guildID,
                    userID: key.userID,
                    xp: key.xp,
    ...
    static async deleteGuild(guildId: string): Promise<any> {
        ...
        await LevelSchema.deleteMany({ guildID: guildId }).catch((e: any): boolean => { return false });
