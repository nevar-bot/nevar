	...
	static configValidator(): void {
		...
		if (!fs.existsSync('./config.toml')) {
			if (fs.existsSync('./config-sample.toml')) {
				...
				Logger.error(
					"TOML: config.toml does not exist. Make sure to run 'npm run build'"
				);
			}
			return process.exit();
		}
		const config: any = toml.parse(
		...
		if (!config.apikeys['AMARI_BOT'])
			Logger.warn(
				"TOML: apikeys.AMARI_BOT is empty. Amari level as a giveaway requirement won't work"
			);
		if (!config.apikeys['TOP_GG'])
			Logger.warn(
				"TOML: apikeys.TOP_GG is empty. Posting stats to discordbotlist.com and receiving votes won't work"
			);
		if (!config.apikeys['TOP_GG_AUTH'])
			Logger.warn(
				"TOML: apikeys.TOP_GG_AUTH is empty. Receiving votes from discordbotlist.com won't work"
			);
		if (!config.apikeys['WEATHER'])
			Logger.warn(
				"TOML: apikeys.WEATHER is empty. Weather command won't work"
			);
		if (!config.apikeys['OPENAI'])
			Logger.warn(
				"TOML: apikeys.OPENAI is empty. Weather command won't work"
			);
		if (!config.apikeys['GOOGLE'])
			Logger.warn(
				"TOML: apikeys.GOOGLE is empty. Weather command won't work"
			);
