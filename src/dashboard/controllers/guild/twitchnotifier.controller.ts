...
export default {
	...
	async get(req: Request, res: Response): Promise<void> {
		...
		/* Render page */
		res.render("guild/twitchnotifier", {
			client: client,
			title: "Twitch Benachrichtigungen",
			module: "twitchnotifier",
	...
	async post(req: Request, res: Response): Promise<void> {
		...
		try{
			/**
			 * TODO
			 * Twitchnotifier durchgehen, und wenn nötig updaten
			 */

			/* Save modified guild data */
			guildData.markModified("settings.notifiers.twitch");
		...
		/* Redirect */
		res.status(200).redirect("/dashboard/" + req.params.guildId + "/twitchnotifier");
	},
};
