	...
	const json: any = {
		...
		res: {
			...
			user_count: client.guilds.cache.reduce(
				(sum: number, guild: any): number => sum + (guild.available ? guild.memberCount : 0),
				0,
			),
			channel_count: client.channels.cache.size,
			command_count: client.commands.size,
			vote_count:
				JSON.parse(fs.readFileSync("./assets/votes.json").toString())[moment().format("MMMM").toLowerCase()] ||
				0,
