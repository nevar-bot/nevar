...
export default class {
	...
	public async dispatch(oldEmoji: any, newEmoji: any): Promise<any> {
		...
		let emojiLogMessage: string = this.client.emotes.edit + " Name: ~~" + oldEmoji.name + "~~ **" + newEmoji.name + "**";

		const auditLogs: any = await guild.fetchAuditLogs({ type: AuditLogEvent["EmojiUpdate"], limit: 1 }).catch((e: any): void => {});
		if (auditLogs) {
