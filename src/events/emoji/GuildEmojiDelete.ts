...
export default class {
	...
	public async dispatch(emoji: any): Promise<any> {
		...
		const auditLogs: any = await guild.fetchAuditLogs({ type: AuditLogEvent["EmojiDelete"], limit: 1 }).catch((): void => {});
		...
		const emojiLogMessage: string =
			" ### " + this.client.emotes.events.emoji.delete + " " + guild.translate("events/emoji/GuildEmojiDelete:deleted")+ "\n\n" +
			properties.join("\n");
