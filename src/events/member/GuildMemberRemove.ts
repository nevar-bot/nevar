...
export default class
{
    ...
    public async dispatch(member: any): Promise<any>
    {
        ...
        const memberLeaveText: string =
            this.client.emotes.edit + " Anzeigename: " + member.user.displayName + "\n" +
            this.client.emotes.user + " Nutzername: " + member.user.username + "\n" +
            this.client.emotes.id + " ID: " + member.id;

        const memberLeaveEmbed: EmbedBuilder = this.client.createEmbed(memberLeaveText, null, "error");
        memberLeaveEmbed.setTitle(this.client.emotes.events.member.ban + "Mitglied hat den Server verlassen");
        ...
        if(guildData.settings?.farewell.enabled){
            ...
            const farewellChannel: any = guild.channels.cache.get(guildData.settings.farewell.channel) || await guild.channels.fetch(guildData.settings.farewell.channel)
                .catch((e: any): void => {
                    const errorText: string =
                        this.client.emotes.user + " Mitglied: " + member.user.username + "\n" +
                        this.client.emotes.arrow + " Kanal: " + guildData.settings.welcome.channel;

                    const errorEmbed: EmbedBuilder = this.client.createEmbed(errorText, null, "error");
                    errorEmbed.setTitle(this.client.emotes.error + " Verabschiedungsnachricht fehlgeschlagen");
            ...
            if(guildData.settings.farewell.type === "embed"){
                ...
                if(guildData.settings.farewell?.thumbnail) farewellEmbed.setThumbnail(member.user.displayAvatarURL());
                return farewellChannel.send({ embeds: [farewellEmbed] }).catch((e: any): void => {
                    const errorText: string =
                        this.client.emotes.user + " Mitglied: " + member.user.username + "\n" +
                        this.client.emotes.arrow + " Kanal: " + farewellChannel.toString();
                ...
                return farewellChannel.send({ content: farewellMessage })
                    .catch((e: any): void => {
                        const errorText: string =
                            this.client.emotes.user + " Mitglied: " + member.user.username + "\n" +
                            this.client.emotes.arrow + " Kanal: " + farewellChannel.toString();
