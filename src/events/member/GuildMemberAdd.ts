...
export default class
{
	...
	public async dispatch(member: any): Promise<any>
	{
		...
		const memberJoinText: string =
			...
			(inviteData.inviter ? this.client.emotes.invite + " Eingeladen von: " + inviteData.inviter.username : "");
		...
		if (guildData.settings?.welcome.enabled) {
			...
			const welcomeChannel: any = guild.channels.cache.get(guildData.settings.welcome.channel) || await guild.channels.fetch(guildData.settings.welcome.channel)
				.catch((e: any): void =>
				{
					const errorText: string =
						this.client.emotes.user + " Mitglied: " + member.user.username + "\n" +
				...
				if (guildData.settings.welcome.type === "embed") {
					...
					welcomeChannel.send({ embeds: [welcomeEmbed] })
						.catch((e: any): void =>
						{
							const errorText: string =
								this.client.emotes.user + " Mitglied: " + member.user.username + "\n" +
								this.client.emotes.arrow + " Kanal: " + welcomeChannel.toString();
			...
			if (inviteData.inviter && inviteData.invite) {
				...
				if (memberData.inviteUsed === inviteData.invite.code) inviterData.invites.find((i: any): boolean => i.code === inviteData.invite.code).fake++;
				if (memberData.inviteUsed === inviteData.invite.code) inviterData.invites.find((i: any): boolean => i.code === inviteData.invite.code).left--;
