...
export default class {
	...
	public async dispatch(oldThread: any, newThread: any): Promise<any> {
		...
		if (oldThread.archived !== newThread.archived)
			properties.push(
				this.client.emotes.quotes + " " + (newThread.archived ? "Thread archiviert" : "Thread unarchiviert"),
			);
		if (oldThread.locked !== newThread.locked)
			properties.push(
				this.client.emotes.quotes + " " + (newThread.locked ? "Thread gesperrt" : "Thread entsperrt"),
			);
		...
		threadLogEmbed.setTitle(
			this.client.emotes.events.thread.update + " " + this.client.channelTypes[newThread.type] + " bearbeitet",
		);
