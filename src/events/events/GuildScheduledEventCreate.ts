...
export default class {
	...
	public async dispatch(scheduledEvent: any): Promise<void> {
		...
		if (scheduledEvent.scheduledStartTimestamp)
			properties.push(this.client.emotes.reminder + " Startzeit: " + moment(scheduledEvent.scheduledStartTimestamp).format("DD.MM.YYYY HH:mm"));
		if (scheduledEvent.scheduledEndTimestamp)
			properties.push(this.client.emotes.reminder + " Endzeit: " + moment(scheduledEvent.scheduledEndTimestamp).format("DD.MM.YYYY HH:mm"));
