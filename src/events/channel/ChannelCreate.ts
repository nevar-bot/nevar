...
export default class {
	...
	public async dispatch(channel: any): Promise<any> {
		...
		if (channel.userLimit)
			properties.push(this.client.emotes.users + " Userlimit: " + (channel.userLimit === 0 ? "unbegrenzt" : channel.userLimit));
		if (channel.videoQualityMode)
			properties.push(this.client.emotes.monitor + " Videoqualität: " + (channel.videoQualityMode === 1 ? "automatisch" : "720p"));
		if (properties.length < 1) return;

		let channelLogMessage: string = properties.join("\n");

		const auditLogs: any = await guild.fetchAuditLogs({ type: AuditLogEvent["ChannelCreate"], limit: 1 }).catch((e: any): void => {});
		if (auditLogs) {
		...
		channelLogEmbed.setTitle(this.client.emotes.events.channel.create + " " + this.client.channelTypes[channel.type] + " erstellt");
