...
export default class
{
    ...
    async dispatch(interaction: any): Promise<any>
    {
        ...
        if(interaction.isContextMenuCommand()){
            ...
            if(!contextMenu){
                const errorMessageEmbed: EmbedBuilder = this.client.createEmbed("Ein unerwarteter Fehler ist aufgetreten, bitte kontaktiere den [Support]{0}.", "error", "error", this.client.support);
                await interaction.reply({ embeds: [errorMessageEmbed], ephemeral: true }).catch((e: any): void => {});
                return this.client.alertException("Context menu " + interaction.commandName + " not found", guild.name, member.user, "<Client>.contextMenus.get(\"" + interaction.commandName + "\")");
            }
            ...
            try {
                ...
                if(!interaction.deferred){
                    const errorMessageEmbed: EmbedBuilder = this.client.createEmbed("Ein unerwarteter Fehler ist aufgetreten, bitte kontaktiere den [Support]{0}.", "error", "error", this.client.support);
                    await interaction.reply({ embeds: [errorMessageEmbed], ephemeral: true }).catch((e: any): void => {});
                    await this.client.alertException("ContextInteraction is not deferred", guild.name, member.user, "<ContextInteraction>.deferReply()");
                }
            }
            ...
            if(time > Date.now()){
                ...
                if(!data.user.staff.state && !this.client.config.general["OWNER_IDS"].includes(member.user.id)){
                    ...
                    const cooldownMessageEmbed: EmbedBuilder = this.client.createEmbed("Du musst noch {0} {1} warten, bis du diesen Befehl erneut nutzen kannst.", "error", "error", seconds, secondsString);
                    return interaction.followUp({ embeds: [cooldownMessageEmbed] });
                }
            }
        ...
        if(interaction.isCommand()){
            ...
            try {
                ...
                if(!interaction.deferred){
                    const errorMessageEmbed: EmbedBuilder = this.client.createEmbed("Ein unerwarteter Fehler ist aufgetreten, bitte kontaktiere den [Support]{0}.", "error", "error", this.client.support);
                    await interaction.reply({ embeds: [errorMessageEmbed], ephemeral: true }).catch((e: any): void => {});
                    await this.client.alertException("CommandInteraction is not deferred", guild.name, member.user, "<CommandInteraction>.deferReply()");
                }
            }
            ...
            for(const neededBotPermission of command.conf.botPermissions){
                // @ts-ignore
                if(!channel.permissionsFor(guild.members.me).has(PermissionsBitField.Flags[neededBotPermission])){
                    neededBotPermissions.push(this.client.permissions[neededBotPermission]);
                }
            }
            if(neededBotPermissions.length > 0){
                const missingPermissionMessageEmbed = this.client.createEmbed("Folgende Berechtigungen fehlen mir, um den Befehl ausführen zu können:\n\n{0} {1}", "error", "error", this.client.emotes.arrow, neededBotPermissions.join("\n" + this.client.emotes.arrow + " "));
                return interaction.followUp({ embeds: [missingPermissionMessageEmbed] });
            }

            /* Command is nsfw */
            if(!channel.nsfw && command.conf.nsfw){
                const nsfwMessageEmbed: EmbedBuilder = this.client.createEmbed("Dieser Befehl kann nur in altersbeschränkten Kanälen verwendet werden.", "error", "error");
            ...
            const disabledCommandsJson: any = JSON.parse(fs.readFileSync("./assets/disabled.json"));
