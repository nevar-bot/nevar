...
import { ActionRowBuilder, AnyComponentBuilder, ButtonBuilder, EmbedBuilder } from "discord.js";
...
export default class {
	...
	public async dispatch(interaction: any, data: any, guild: any, suggestion: any, image: any = null): Promise<any> {
		...
		const voteButtonComponentsRow: ActionRowBuilder<AnyComponentBuilder> = this.client.createMessageComponentsRow(
			buttonYes,
			buttonNo,
		);
		...
		const suggestionVoteEmbedMessage: any = await suggestionChannel
			.send({
				embeds: [suggestionVoteEmbed],
				components: [voteButtonComponentsRow],
			})
			.catch((e: any): void => {});
		...
		reviewEmbed.setTitle(
			"Idee von " + interaction.member.user.displayName + " (@" + interaction.member.user.username + ")",
		);
		...
		const buttonAccept: ButtonBuilder = this.client.createButton(
			"review_suggestion_" + suggestionVoteEmbedMessage.id + "_" + suggestionChannel.id + "_accept",
			"Annehmen",
			"Success",
			"success",
		);
		const buttonDecline: ButtonBuilder = this.client.createButton(
			"review_suggestion_" + suggestionVoteEmbedMessage.id + "_" + suggestionChannel.id + "_decline",
			"Ablehnen",
			"Danger",
			"error",
		);
		const reviewButtonComponentsRow: ActionRowBuilder<AnyComponentBuilder> = this.client.createMessageComponentsRow(
			buttonAccept,
			buttonDecline,
		);
		...
		await reviewChannel
			.send({
				embeds: [reviewEmbed],
				components: [reviewButtonComponentsRow],
			})
			.catch(() => {});
	}
}
