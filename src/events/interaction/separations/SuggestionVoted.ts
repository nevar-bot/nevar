...
export default class {
	...
	public async dispatch(interaction: any, customId: any, data: any): Promise<any> {
		...
		const userHasAlreadyVoted: any =
			data.member.suggestions.find((s: any): any => s.id === suggestionEmbedMessage.id) || null;
		...
		if (type === "yes") {
			...
			newDownVotes = parseInt(
				String(userHasAlreadyVoted ? parseInt(currentDownVotes) - 1 : parseInt(currentDownVotes)),
			);
		} else if (type === "no") {
			newUpVotes = parseInt(
				String(userHasAlreadyVoted ? parseInt(currentUpvotes) - 1 : parseInt(currentUpvotes)),
			);
		...
		suggestionEmbedMessage.edit({
			embeds: [suggestionEmbedMessage.embeds[0]],
			components: [suggestionEmbedMessage.components[0]],
		});
