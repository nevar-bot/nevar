...
export default class {
	...
	public async dispatch(
		...
		const userHasAlreadyVoted: any =
			data.member.suggestions.find(
				(s: any): any => s.id === suggestionEmbedMessage.id
			) || null;
		...
		const currentEmbedFooter: any =
			suggestionEmbedMessage.embeds[0].footer.text;
		...
		if (type === 'yes') {
			...
			newUpVotes = parseInt(
				String(
					userHasAlreadyVoted
						? parseInt(currentUpvotes) - 1
						: parseInt(currentUpvotes)
				)
			);
