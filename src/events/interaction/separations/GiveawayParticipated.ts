...
export default class {
    ...
    public async dispatch(interaction: any, customId: any, data: any, guild: any): Promise<any> {
        ...
        if(giveaway.entrantIds.includes(interaction.user.id)){
            await interaction.reply({ content: "### " + this.client.emotes.tada + " Du nimmst absofort nicht mehr an dem Gewinnspiel teil!", ephemeral: true });
            await this.client.giveawayManager.removeEntrant(interaction.message.id, interaction.user.id);
        }else{
            await interaction.reply({ content: "### " + this.client.emotes.tada + " Du nimmst absofort an dem Gewinnspiel teil!", ephemeral: true });
