...
export default class
{
	...
	public async dispatch(guild: any): Promise<any>
	{
		...
		const firstChannel: any = guild.channels.cache.find((c: any) => (c.type === ChannelType.GuildText || c.type === ChannelType.GuildAnnouncement) && c.permissionsFor(guild.members.me).has(PermissionsBitField.Flags.SendMessages));

		const welcomeMessage: string =
			this.client.emotes.arrow + " Eine **Übersicht meiner Befehle** erhältst du mit {0}\n" +
			this.client.emotes.arrow + " Unten sind zusätzlich einige **hilfreiche Links** zu finden.\n\n" +
		...
		const welcomeMessageEmbed: EmbedBuilder = this.client.createEmbed(welcomeMessage, null, "normal", (helpCommand ? "</help:" + helpCommand + ">" : "/help"));
		...
		const buttonSupport: ButtonBuilder = this.client.createButton(null, "Support", "Link", "discord", false, this.client.config.support["INVITE"]);
		...
		const buttonVote: ButtonBuilder = this.client.createButton(null, "Voten", "Link", "heart", false, "https://discordbotlist.com/bots/" + this.client.user!.id + "/upvote");
		const buttonDonate: ButtonBuilder = this.client.createButton(null, "Unterstützen", "Link", "gift", false, "https://prohosting24.de/cp/donate/nevar");
