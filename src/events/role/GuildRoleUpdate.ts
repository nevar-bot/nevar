...
export default class {
	...
	public async dispatch(oldRole: any, newRole: any): Promise<any> {
		...
		if (oldRole.color !== newRole.color)
			properties.push(this.client.emotes.settings + " Farbe: ~~" + oldRole.hexColor + "~~ **" + newRole.hexColor + "**");
		if (oldRole.name !== newRole.name) properties.push(this.client.emotes.edit + " Name: ~~" + oldRole.name + "~~ **" + newRole.name + "**");
		...
		let roleLogMessage: string = this.client.emotes.ping + " Rolle: " + newRole.toString() + "\n" + properties.join("\n");
