...
export default class {
	...
	public async dispatch(role: any): Promise<any> {
		...
		const auditLogs: any = await guild.fetchAuditLogs({ type: AuditLogEvent["GuildRoleCreate"], limit: 1 }).catch((): void => {});
		...
		const roleLogMessage: string =
			" ### " + this.client.emotes.events.role.create + " " + guild.translate("events/role/GuildRoleCreate:created")+ "\n\n" +
			properties.join("\n");
