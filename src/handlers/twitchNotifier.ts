...
export default {
    init(client: BaseClient): void {
        const authProvider: AppTokenAuthProvider = new AppTokenAuthProvider(client.config.apikeys["TWITCH_CLIENT_ID"], client.config.apikeys["TWITCH_CLIENT_SECRET"]);
                    ...
                    if (guildData.settings?.notifiers?.twitch?.enabled) {
                        for (let channel of guildData.settings.notifiers.twitch.channels) {
                            ...
                            if (stream && channel.lastStreamId !== stream.id) {
                                guildData.settings.notifiers.twitch.channels.find((c: any): boolean => c.id === channel.id).lastStreamId =
                                    stream.id;
