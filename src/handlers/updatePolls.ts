           ...
           client.guilds.cache.forEach((guild: Guild): void => {
               client.findOrCreateGuild(guild.id).then((guildData: any): void => {
                   ...
                   for(const poll of guildData.settings?.polls){
                       ...
                       channel.messages.fetch(messageId).then(async (pollMessage: any): Promise<void> => {
                           ...
                           const totalVoteCount: number = pollVotes.reduce((total: number, emoji: any) => total + emoji.count, 0) - pollVotes.length;
