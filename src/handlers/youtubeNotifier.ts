...
export default
	{
		init(client: BaseClient): void
		{
			...
			{
				await Promise.all(client.guilds.cache.map(async (guild: Guild): Promise<void> =>
				{
					...
					if (guildData.settings?.notifiers?.youtube?.enabled) {
						for (let channel of guildData.settings.notifiers.youtube.channels) {
							...
							if (lastVideo && channel.lastVideoId !== lastVideo.id) {
								guildData.settings.notifiers.youtube.channels.find((c: any): boolean => c.id === channel.id).lastVideoId = lastVideo.id;
								...
								if (announcementChannel) {
									const announcementEmbed: EmbedBuilder = client.createEmbed("### {0} [{1}]({2})", null, "normal", client.emotes.link, lastVideo.title, lastVideo.url);
